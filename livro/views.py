...
def cadastrar_categoria(request):
    ...
    if int(id_usuario) == int(request.session.get('usuario')):
        ...
        return HttpResponse('ERRO de segurança. Pare de ser um Usuario malandrinho, não foi dessa vez!!!')
...
def alterar_livro(request):
    ...
    if livro.usario.id == request.session['usuario']:
...
def seus_emprestimos(request):
    ...
    return render(request, 'seus_emprestimos.html', {'usuario_logato': request.session['usuario'],
                                                    'emprestimos': emprestimos})